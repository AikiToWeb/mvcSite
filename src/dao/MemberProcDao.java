...
public class MemberProcDao {
	...
	public int memberInsert(MemberInfo memberInfo, MemberAddr memberAddr) {
		// 사용자가 입력한 정보들로 회원가입을 하는 메소드
		PreparedStatement pstmt = null;	// ResultSet 필요 없음
		int result = 0;
		
		try {
			String sql = "insert into t_member_info (mi_id, mi_pw, mi_name, mi_gender, mi_birth, mi_phone, mi_email, mi_isad, mi_point) " + 
					" values (?, ?, ?, ?, ?, ?, ?, ?, ?)";
			pstmt = conn.prepareStatement(sql);	// prepare'd'Statment인데 만들 때는 prepareStatement(sql)로 나옴
			...
			if(result > 0) {	// 정상적으로 t_member_info 테이블에 레코드가 추가되었을 경우
				...
				if(result > 0) {	// 정상적으로 t_member_info 테이블에 레코드가 추가되었을 경우
					...
					pstmt.setInt(3, 1000);
					pstmt.setString(4, "가입축하금");
					result = pstmt.executeUpdate();
			}
		}
			
		} catch (Exception e) {
			System.out.println("MemberProcDao의 memberInsert() 오류 발생");
			e.printStackTrace();
		}
		return result;
	}
