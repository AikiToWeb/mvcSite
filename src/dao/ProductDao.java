	...
	// 검색된 상품 목록을 ArrayList<ProductInfo>에 담아 리턴하는 메소드
		...
		try {
			...
			while (rs.next()) {
				...
				pdt.setPi_isopt(rs.getString("pi_isopt"));				pdt.setPi_img1(rs.getString("pi_img1"));
		...
		public ArrayList<PdtCataSmall> getCataSmallList() {
			// 대분류 목록을 ArrayList<PdtCataSmall>에 담아 리턴하는 메소드
				Statement stmt = null;
				ResultSet rs = null;
		...
		public ArrayList<PdtBrandInfo> getBrandInfoList() {
			// 대분류 목록을 ArrayList<PdtBrandInfo>에 담아 리턴하는 메소드
				Statement stmt = null;
				ResultSet rs = null;
